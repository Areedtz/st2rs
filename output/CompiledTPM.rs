...
fn sign(a1: bytes, a2: skey) -> bytes {
	...
	return a2.sign(padding, &a1[..]).expect("failed to encrypt")
}
...
fn senc(a1: bytes, a2: symkey) -> bytes {
	...
	return encrypt(Cipher::aes_128_cbc(), &a2, Some(&iv), &a1[..]).unwrap()
}
...
fn hash(a1: bytes, a2: bytes) -> bytes {
	...
	let buf: [u8; 64] = a.try_into()
        .unwrap_or_else(|a: Vec<u8>| panic!("Expected a Vec of length {} but it was {}", 64, a.len()));

	return openhash(MessageDigest::sha256(), &buf).unwrap().to_vec();
}
...
mod tests {
	...
	fn test_equation_0() {
		let k = fresh_skey();
		let m = fresh_bytes();
		assert_eq!(checksign(sign(m, k), pk(k)), m);
	}
	#[test]
	fn test_equation_1() {
		let k = fresh_skey();
		let m = fresh_bytes();
		assert_eq!(adec(aenc(m, pk(k)), k), m);
	}
	#[test]
	fn test_equation_2() {
		let k = fresh_symkey();
		let m = fresh_bytes();
		assert_eq!(sdec(senc(m, k), k), m);
	}
	#[test]
	fn test_equation_3() {
		let k = fresh_symkey();
		assert_eq!(symkeyunwrapper(symkeywrapper(k)), k);
	}
	#[test]
	fn test_equation_4() {
		let b1 = fresh_bytes();
		let b2 = fresh_bytes();
		assert_eq!(bytesbytesunwrapper(bytesbyteswrapper(b1, b2)), (b1, b2));
	}
	#[test]
	fn test_equation_5() {
		let b = fresh_bytes();
		let k = fresh_pkey();
		assert_eq!(pkeybytesunwrapper(pkeybyteswrapper(k, b)), (k, b));
	}
}
...
fn alice(c_AliceTPM: Chan<(), AliceTPM>, c_AliceParent: Chan<(), AliceParent>, tpmencpk: pkey, tpmsignpk: pkey) {
	...
	let c_AliceTPM = send(c_AliceTPM, senc(bytesbyteswrapper(na, sesid_a), sesk_a));
	let h_obt = hash(OBT(), hash(na, pcr));
	let c_AliceTPM = send(c_AliceTPM, h_obt);
...
fn parent(c_ParentAlice: Chan<(), ParentAlice>, c_ParentTPM: Chan<(), ParentTPM>, tpmencpk: pkey, tpmsignpk: pkey) {
	...
	let (c_ParentTPM, sesid_a) = recv(c_ParentTPM);
	let c_ParentTPM = send(c_ParentTPM, envlp);
	// Need to make a choice on c_ParentTPM. Either c_ParentTPM.sel1() or c_ParentTPM.sel2()
	/*
	let c_ParentTPM = c_ParentTPM.sel1();
	let (c_ParentTPM, quote) = recv(c_ParentTPM);
	close(c_ParentTPM);
	close(c_ParentAlice);
	*/
...
fn tpm(c_TPMAlice: Chan<(), TPMAlice>, c_TPMParent: Chan<(), TPMParent>, tpmencsk: skey, tpmsignsk: skey) {
	...
	if sesid_a == v1 {
		...
		let c_TPMAlice = send(c_TPMAlice, sign(pkeybyteswrapper(pk(&genkey(tpmencsk, h_obt)), h_obt), tpmsignsk));
		let (c_TPMParent, enc_sesk_p) = recv(c_TPMParent);
		let sesid_a = fresh_bytes();
		let c_TPMParent = send(c_TPMParent, sesid_a);
...
fn main() {
	let tpmencsk = fresh_skey();
	let tpmsignsk = fresh_skey();
	...
	let parent_t = thread::spawn(move || parent(c_ParentAlice, c_ParentTPM, tpmencpk, tpmsignpk));
